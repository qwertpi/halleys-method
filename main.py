...
class function():
    ...
    def second_derivative(self, x):
        return self.power * (self.power - 1) * self.a * (x**(self.power - 2))
...
while True:
    numerator = 2 * func.calc(x) * func.derivative(x)
    denominator = (2 * (func.derivative(x)) ** 2) - (func.calc(x) * func.second_derivative(x))
